# Phase 17：多輪精煉（Plan Refinement）

## Context

Phase 14 的結構化輸出讓行程資料變成可操作的 JSON。Phase 17 利用這份資料實現「局部修改」：用戶說「太貴了」，系統只重新查住宿，不把整份行程推翻重來。

這是讓系統從「工具」演化為「對話式旅遊顧問」的核心功能。

**先決條件**：Phase 14（結構化輸出 + PlanState 基礎型別）必須先完成。
...
清除 PlanState，走完整的三 agent agentic loop（同 Phase 8 邏輯）。
...
| `web/src/pages/ChatPage.tsx` | 修改 | 每次 artifact 更新時，同步更新地圖（Phase 16）|
...
| 5 | `orchestratorExecutor.ts` — `full_replan` case（清除 state + 重走 Phase 8 流程）|
...
| Agent 修改後回傳 schema 不符 | SchemaValidator（Phase 14）觸發，回傳舊版 PlanState + error message |
